...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        EXIT_THRESHOLD = 0.7 * STD

        BETA = 1.824590
        ALPHA = 0.3 # convex combination of moving average and precalculated mean
        mean = price_history_COCONUT[-100:-1].mean() - price_history_COCONUT_COUPON[-100:-1].mean() * BETA
        mean = 8841.201392747003 * (1-ALPHA) + mean * ALPHA
                    ...
                    if (hold_history[-1] < 0 and spread < EXIT_THRESHOLD) or (hold_history[-1] > 0 and spread > -EXIT_THRESHOLD):
                        if hold_history[-1] > 0:
                            if len(o_depth.buy_orders) != 0:
                                ...
                                orders.append(Order(product, best_bid, (int) ((-np.sign(best_bid_vol) * np.minimum(np.abs(best_bid_vol), np.abs(state.position['COCONUT'])))) ))
                        else:
                            if len(o_depth.sell_orders) != 0:
                                ...
                                orders.append(Order(product, best_ask, (int) ((-np.sign(best_ask_vol) * np.minimum(np.abs(best_ask_vol), np.abs(state.position['COCONUT'])))) ))

                # Entry
                if hold_history[-1] == 0 and state.timestamp <= 985_000:
                    if spread > ENTRY_THRESHOLD: #sell coconut
                        if len(o_depth.buy_orders) != 0:
                            ...
                            orders.append(Order(product, best_bid, (int) (-np.sign(best_bid_vol) * np.minimum(np.abs(best_bid_vol), max_sell)) )) #sell until position reaches limit
                    ...
                    if spread < -ENTRY_THRESHOLD: #buy coconut
                        if len(o_depth.sell_orders) != 0:
                            ...
                            orders.append(Order(product, best_ask, (int) (-np.sign(best_ask_vol) * np.minimum(np.abs(best_ask_vol), max_buy)) )) #buy until position reaches limit
                
                # Exit at end
                if state.timestamp >= 985_000:
                    hold_history[-1] = (int) (np.sign(state.position['COCONUT']))
                    ...
                    if (hold_history[-1] > 0 and spread > -EXIT_THRESHOLD) or (hold_history[-1] < 0 and spread < EXIT_THRESHOLD):
                        if hold_history[-1] < 0: #sell coupon
                            if len(o_depth.buy_orders) != 0:
                                ...
                                orders.append(Order(product, best_bid, (int) (-np.sign(best_bid_vol) * np.minimum(np.abs(best_bid_vol), np.abs(state.position['COCONUT_COUPON']))) ))
                        else:
                            if len(o_depth.sell_orders) != 0: #buy coupon
                                best_ask = min(o_depth.sell_orders.keys())
                                best_ask_vol = o_depth.sell_orders[best_ask]

                                orders.append(Order(product, best_ask, (int) (-np.sign(best_ask_vol) * np.minimum(np.abs(best_ask_vol), np.abs(state.position['COCONUT_COUPON']))) ))
